...
import { hexToHsl, hslToHex, normalizeHex } from "./colorMath";
import { BIAS_MODES, PaletteItem, GeneratedSwatch, HarmonyRule, HslColor } from "../types";
...
export function getPaletteHslList(palette: PaletteItem[]): HslColor[] {
  if (!palette || palette.length === 0) {
    return [{ h: 218, s: 100, l: 59 }];
  }
  const result: HslColor[] = [];
  for (const p of palette) {
    const norm = normalizeHex(p.hex);
    if (norm) {
      result.push(hexToHsl(norm));
    }
  }
  return result.length > 0 ? result : [{ h: 218, s: 100, l: 59 }];
}

export function generateHarmoniousPalette(
  palette: PaletteItem[],
  biasMode: string = BIAS_MODES.ANY,
  count: number = 8
): GeneratedSwatch[] {
  const hslList = getPaletteHslList(palette);
  const results: GeneratedSwatch[] = [];
  const generatedHexes = new Set<string>();

  for (const p of palette) {
    const norm = normalizeHex(p.hex);
    if (norm) generatedHexes.add(norm);
  }
  ...
  while (results.length < count && attempts < maxAttempts) {
    ...
    if (!generatedHexes.has(hex)) {
      ...
      results.push({
        id: `gen-${Date.now()}-${results.length}-${Math.random().toString(36).substring(2, 7)}`,
        hex,
        h: targetHue,
        s: targetSat,
        l: targetLight,
  ...
  while (results.length < count) {
    ...
    results.push({
      id: `gen-fb-${results.length}-${Math.random().toString(36).substring(2, 7)}`,
